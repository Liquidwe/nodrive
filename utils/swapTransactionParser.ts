...
interface Instruction {
  ...
  args?: {
    ...
    base_amount_out?: string;
    min_quote_amount_out?: string;
    max_quote_amount_in?: string;
    amount?: string;
  };
}
...
interface ParsedInstruction {
  instructions: {
    pumpFunIxs: Instruction[];
  };
  inner_ixs: Instruction[];
}

export function parseSwapTransactionOutput(
  ...
  transaction: VersionedTransactionResponse
    ...
    const swapInstruction = parsedInstruction.instructions.pumpFunIxs.find(
        (instruction: Instruction) => instruction.name === 'buy' || instruction.name === 'sell'
    );
    ...
    const signerPubkey = swapInstruction.accounts.find((account: Account) => account.name === 'user')?.pubkey;
    const user_quote_token_account = swapInstruction.accounts.find((account: Account) => account.name === "user_quote_token_account")?.pubkey;
    const pool_base_token_account = swapInstruction.accounts.find((account: Account) => account.name === "pool_base_token_account")?.pubkey;
    ...
    const determineBuySellEvent = () => {
        const baseMintPubkey = swapInstruction.accounts.find((account: Account) => account.name === 'base_mint')?.pubkey;
        const quoteMintPubkey = swapInstruction.accounts.find((account: Account) => account.name === 'quote_mint')?.pubkey;
